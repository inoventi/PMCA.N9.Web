        ...
        toExcel: function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.isSuccess) {
                        var path = '/Gantt/DownloadFile?filePath=' + data.valueString + '&projectName=' + data.valueString1;
...
$(function () {
    ...
    nIntervId = setInterval(chechLoader, 1000);
});

function chechLoader() {  
    if (window.ajax_loading) { 
        $("#gantt").css({ 'pointer-events': 'none' });
        $("body").css("cursor", "progress");
        $("#loader").attr("hidden", false); 
    } else {
        $("#loader").attr("hidden", true);
        $("#gantt").css({ 'pointer-events': '' });
